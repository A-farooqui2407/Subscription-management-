...
const Reports = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Default to showing all initially
      setFilteredInvoices(iRes);

    } catch (e) {
      toast.error('Failed to parse active ledgers natively for reports');
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  const applyFilters = () => {
      ...
      if (customerId !== '') f = f.filter(i => String(i.customerId) === String(customerId));
      
      if (startDate !== '') {
          const sd = new Date(startDate);
          f = f.filter(i => new Date(i.issueDate) >= sd);
      }
      
      if (endDate !== '') {
          const ed = new Date(endDate);
          f = f.filter(i => new Date(i.issueDate) <= ed);
      }
      
      setFilteredInvoices(f);
      toast.success("Architectural filters executed.");
  };
  ...
  const totals = filteredInvoices.reduce((acc, inv) => {
      ...
      acc.tax += inv.taxTotal || 0;
      acc.total += inv.total || 0;
      acc.collected += inv.amountPaid || 0;
      return acc;
  }, { subtotal: 0, tax: 0, total: 0, collected: 0 });
      ...
      {!loading && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 duration-300">
             <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-3xl shadow-sm text-center relative overflow-hidden">
                 ...
                 <p className="text-sm font-bold tracking-widest uppercase text-indigo-800">Filtered Yield Arrays</p>
                 <h2 className="text-4xl font-black text-indigo-600 mt-2 tracking-tighter">{filteredInvoices.length} Found</h2>
             </div>
             ...
             <div className="bg-blue-50 border border-blue-100 p-6 rounded-3xl shadow-sm text-center relative overflow-hidden">
                 ...
                 <h2 className="text-4xl font-black text-blue-600 mt-2 tracking-tighter font-mono">${totals.total.toFixed(2)}</h2>
             </div>
             ...
             <div className="bg-green-50 border border-green-100 p-6 rounded-3xl shadow-sm text-center relative overflow-hidden">
                 ...
                 <h2 className="text-4xl font-black text-green-600 mt-2 tracking-tighter font-mono">${totals.collected.toFixed(2)}</h2>
             </div>
          </div>
      )}
             ...
             <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                    {filteredInvoices.map(inv => (
                        <tr key={inv.id} className="hover:bg-slate-50/50 cursor-pointer" onClick={() => navigate(`/invoices/${inv.id}`)}>
                            <td className="p-4 pl-6 font-bold text-indigo-600 text-sm whitespace-nowrap hover:underline">{inv.invoiceNumber}</td>
                            <td className="p-4 text-sm font-medium text-slate-700">{contactsDict[inv.customerId] || 'Client Unknown'}</td>
                            <td className="p-4 text-sm text-slate-500">{inv.issueDate}</td>
                            <td className="p-4 text-center">
                               <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-widest border
                                  ${inv.status === 'paid' ? 'bg-green-100 text-green-700 border-green-200' : 
                                    inv.status === 'confirmed' ? 'bg-orange-100 text-orange-700 border-orange-200' : 
                                    inv.status === 'cancelled' ? 'bg-red-100 text-red-700 border-red-200' : 'bg-slate-100 text-slate-600 border-slate-200'
                                   }
                               `}>
                                   {inv.status}
                               </span>
                            </td>
                            <td className="p-4 text-right text-sm font-medium font-mono text-slate-600">${(inv.subtotal || 0).toFixed(2)}</td>
                            <td className="p-4 text-right text-sm font-medium font-mono text-slate-500">${(inv.taxTotal || 0).toFixed(2)}</td>
                            <td className="p-4 pr-6 text-right text-sm font-black font-mono text-slate-900">${(inv.total || 0).toFixed(2)}</td>
                        </tr>
                    ))}
                    {filteredInvoices.length === 0 && (
                        <tr>
                            <td colSpan="7" className="p-12 text-center text-slate-400 font-medium">Zero matching logic states executed against parameters natively.</td>
                        </tr>
                    )}
                </tbody>
                <tfoot>
                   <tr className="bg-slate-50/80 border-t-2 border-slate-200 text-sm font-black">
                       <td colSpan="4" className="p-4 pl-6 text-right uppercase tracking-widest text-slate-600">Total Filtered Resolution</td>
                       ...
                       <td className="p-4 pr-6 text-right font-mono text-blue-700 tracking-tighter text-lg">${totals.total.toFixed(2)}</td>
                   </tr>
                </tfoot>
             </table>
          </div>
